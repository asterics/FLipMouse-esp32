    ...
    switch(event) {
        ...
        case ESP_GATTS_CONNECT_EVT:
            ...
            if(param->connect.conn_id != 0) {
			   ...
			   ESP_LOGI(LOG_TAG,"the connection establish, conn_id = %x",param->connect.conn_id);
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            if (param->add_attr_tab.num_handle == BAS_IDX_NB && 
                ...
                ESP_LOGD(LOG_TAG,"%s(), start added the hid service to the stack database. incl_handle = %d", 
            ...
            if (param->add_attr_tab.num_handle == HIDD_LE_IDX_NB && 
                ...
                ESP_LOGD(LOG_TAG,"hid svc handle = %x",hidd_le_env.hidd_inst.att_tbl[HIDD_LE_IDX_SVC]);
                hid_add_id_tbl();
...
void hidd_set_attr_value(uint16_t handle, uint16_t val_len, const uint8_t *value)
{
    ...
    if(hidd_inst->att_tbl[HIDD_LE_IDX_HID_INFO_VAL] <= handle && 
        ...
        ESP_LOGE(LOG_TAG,"%s error:Invalid handle value.",__func__);
    }
    return;
}
