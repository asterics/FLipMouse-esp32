...
void esp_hidd_send_keyboard_value(uint16_t conn_id, key_mask_t special_key_mask, uint8_t *keyboard_cmd, uint8_t num_key)
{
    ...
    ESP_LOGD(HID_LE_PRF_TAG, "the key values = %d,%d,%d, %d, %d, %d,%d, %d", buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7]);
    hid_dev_send_report(hidd_le_env.gatt_if, conn_id,
                        HID_RPT_ID_KEY_IN, HID_REPORT_TYPE_INPUT, HID_KEYBOARD_IN_RPT_LEN, buffer);
    return;
}
void esp_hidd_send_joystick_value(uint16_t conn_id, uint8_t *joystick_cmd, uint8_t len)
{
    if (len != HID_JOYSTICK_IN_RPT_LEN) {
        ESP_LOGE(HID_LE_PRF_TAG, "%s(), need exactly %d bytes for joystick", __func__, HID_JOYSTICK_IN_RPT_LEN);
        return;
    }
   
    uint8_t buffer[HID_JOYSTICK_IN_RPT_LEN] = {0};
    memcpy(buffer,joystick_cmd,HID_JOYSTICK_IN_RPT_LEN);
...
void esp_hidd_send_mouse_value(uint16_t conn_id, uint8_t mouse_button, int8_t X, int8_t Y, int8_t wheel)
{
